...
public class DescripcionIncidenciaController {
    ...
    @PostMapping("/tramite")
    public ResponseEntity<DescripcionIncidencia> saveTramite(@ModelAttribute("descripcionIncidencia") DescripcionIncidencia descripcionIncidencia, @RequestParam(value = "archivo", required = false) MultipartFile archivo) {
        try {
            ...
            if (correoRemitente != null){
                if(!correoRemitente.isEmpty()) {
                    ...
                    if (!matcher.find()) {
                        ...
                        EmailService.sendEmail("SE ACTUALIZÓ A TRÁMITE SU INCIDENCIA", "INCIDENCIA ACTUALIZADA", incidencia.getOrigen().getOrigen(), incidencia.getMotivo().getMotivo(), historialIncidencia.getPersona_asignado().getNombre() + " " + historialIncidencia.getPersona_asignado().getApellido(), incidencia.getFecha().toLocalDateTime().format(formatter), correoRemitente);
                    }
                }else {
    ...
    public ResponseEntity<DescripcionIncidencia> saveAtendido(@ModelAttribute("descripcionIncidencia") DescripcionIncidencia descripcionIncidencia, @RequestParam(value = "archivo", required = false) MultipartFile archivo) {
        try {
            Incidencia incidencia = incidenciaService.findById(descripcionIncidencia.getIncidencia().getIdIncidencia()).get();
            HistorialIncidencia historialIncidencia = historialIncidenciaService.findByIncidenciaAndEstado(incidencia,'A');
            String correoRemitente = historialIncidencia.getIncidencia().getPersona().getCorreo();
    ...
    @PutMapping("/update")
    public ResponseEntity<DescripcionIncidencia> update(@ModelAttribute("descripcionIncidencia") DescripcionIncidencia descripcionIncidencia, @RequestParam(value = "archivo", required = false) MultipartFile archivo) {
        try {
            Incidencia incidencia = incidenciaService.findById(descripcionIncidencia.getIncidencia().getIdIncidencia()).get();
            HistorialIncidencia historialIncidencia = historialIncidenciaService.findByIncidenciaAndEstado(incidencia,'A');
            String correoRemitente = historialIncidencia.getIncidencia().getPersona().getCorreo();
