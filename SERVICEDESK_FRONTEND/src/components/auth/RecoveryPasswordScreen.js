...
import { validadorUsuario, validarUsuarioExiste } from '../../actions/auth';
...
export const RecoveryPasswordScreen = () => {
    ...
    const StartDni = async (numeroDocumento, codigoVerificacion, fechaNacimiento) => {
        ToastChakra('CONSULTANDO DATA', 'Espere un momento por favor...!', 'loading', 2000, 'top-right');
        try {
            ...
            if ((response.data[0] === numeroDocumento) && (response.data[1] === codigoVerificacion) && (fechax === fechaNacimiento)) {
                ...
                history.push('/auth/recovery-password/validate');
            } else {
    ...
    const HandleValidatorUser = () => {
        validarUsuarioExiste(validadorDni.dni)
            .then((value) => {
                if (value === true) {
                    dispatch(StartDni(
                        validadorDni.dni,
